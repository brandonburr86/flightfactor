...
void menu_list_flight_codes() {
    // We'll track unique flight codes by scanning flightNoArr
    // For large data, a more efficient approach might be used, but
    // for simplicity, we'll just do a basic "has it been printed?" approach.
...
void menu_predict_flight_code() {
    ...
    // We'll accumulate predictions for all rows that match userCode
    double sumLoad = 0.0;
